...
function renderHome(){
  ...
  const sub = n===0 ? "Start the first job from a project type below. It is saved as you go, and comes back here to be reopened, revised and issued."
                    : "Open a job to carry on where you left off, or start another from a project type.";
  ...
  /* the desk holds two lists: what is on it, and what a new job can be started from. With
     nothing saved there is no choice to make, so the types stand alone under their own heading
     and the hero says so — an empty list is not a view worth a tab. */
  const deskTabs=[{id:"jobs", n:"Jobs", c:String(n)}, {id:"types", n:"Project types", c:String(TYPES.length)}];
  const strip=n>0;
  const cur=strip ? (S.tab==="types"?"types":"jobs") : "types";
  el("home").innerHTML=`
    <section class="desk">
      <div>
    ...
    <section class="section">
      ${strip?`<div class="stagetabs" role="tablist">`+deskTabs.map(t=>
        `<button class="stab${cur===t.id?" cur":""}" data-stab="${t.id}" role="tab" aria-selected="${cur===t.id}">${esc(t.n)} <span class="stn">${esc(t.c)}</span></button>`).join("")+`</div>`
        :`<div class="sechead"><h2>Start a new job</h2><span class="aside">Eight project types · England</span></div>`}
      ${cur==="jobs" ? `<div class="joblist">${jobs.map(jobRow).join("")}</div>`
  ...
  const ht=el("hometiles");
  if(ht) ht.querySelectorAll("button:not(:disabled)").forEach(b=>b.onclick=()=>newJob(b.dataset.k));
  el("home").querySelectorAll(".jobrow").forEach(b=>b.onclick=()=>openJob(b.dataset.id));
  el("home").querySelectorAll("[data-stab]").forEach(t=>t.onclick=()=>{ S.tab=t.dataset.stab; renderHome(); });
}
function jobRow(j){
