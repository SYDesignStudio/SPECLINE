...
function mfrSubstitute(b, mfrId){
  ...
  const tail = ok
    ...
    : `NOTE — Insulation manufacturer: ${M.n} is selected for this job. ${swaps.join("; ")}. At its greatest published thickness the substituted product calculates at ${Ut} W/m²K to ${std} and does not meet the target of ${mf.lim.toFixed(2)}; the build-up is shown with it for information and is not to be issued until the construction is changed or the original product reinstated.`;
